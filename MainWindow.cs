...
namespace bgpicker2
{
    public partial class GamePicker : Form
    {
        ...
        private void activePlayerList_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (Profile p in _activePlayers)
            {
                if (p.name == activePlayerList.SelectedItem as string)
                {
                    selPlayer = p;
                    break;
                }
        ...
        private void GameList_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (BoardGame bg in CollectionScreen._collectionList)
            {
                if (bg.GetName() == GameList.SelectedItem as string)
                {
                    selGame = bg;
                    break;
                }
        ...
        private void importFromBGGTestToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ImportCollectionPrompt icp = new ImportCollectionPrompt();
            icp.ShowDialog();
            LoadGamesList();
        }
    }
